...
schema rules as specific ConfigErrors, legacy auto-registration (R-1), the
TOML emitter round trip, and the config dataclasses' own helpers (the routing
guide lives in the llm-redact-pro package; the shapes are core).

Keyless and pro-free by construction: everything here is `llm_redact.config`."""
...
# TOML spells nan/inf natively; an int beyond float range overflows float().
# Every one of these used to pass `serve --check` and then break /status
# (Starlette serializes with allow_nan=False), the dashboard, and the editor
# (nan != nan defeats the reparse-equality guard).
NON_FINITE = [float("nan"), float("inf"), float("-inf"), 10**400, -(10**400)]


@pytest.mark.parametrize("bad", NON_FINITE)
@pytest.mark.parametrize("key", ["monthly_budget_usd", "monthly_budget_tokens", "cooldown_seconds"])
def test_upstream_numbers_must_be_finite(key: str, bad: float) -> None:
    if key == "monthly_budget_tokens" and not isinstance(bad, int):
        pytest.skip("the integer key rejects floats by type already")
    _err(_upstream_raw(**{key: bad}), rf"\[upstreams.u\] {key} must be a finite number")


@pytest.mark.parametrize("bad", NON_FINITE)
@pytest.mark.parametrize("key", ["request_deadline_seconds", "throttle_retry_max_seconds"])
def test_routing_numbers_must_be_finite(key: str, bad: float) -> None:
    raw = _raw()
    raw["routing"][key] = bad
    _err(raw, rf"\[routing\] {key} must be a finite number")


@pytest.mark.parametrize("bad", NON_FINITE)
@pytest.mark.parametrize("field", ["input", "output", "cache_read", "cache_write"])
def test_price_override_rates_must_be_finite(field: str, bad: float) -> None:
    entry: dict[str, Any] = {"input": 1, "output": 2, field: bad}
    _err(
        {"prices": {"override": {"m": entry}}},
        rf"\[prices.override.'m'\] {field} must be a finite number",
    )


def test_non_finite_error_never_echoes_the_value() -> None:
    # The message names section and key only — the same discipline as the
    # type errors (a budget is not a secret, but the helper is shared).
    with pytest.raises(ConfigError) as info:
        parse_config(_upstream_raw(monthly_budget_usd=float("inf")), "<t>")
    assert "inf" not in str(info.value).replace("(nan/inf are not accepted)", "")
    with pytest.raises(ConfigError) as info:
        parse_config(_upstream_raw(monthly_budget_tokens=10**400), "<t>")
    assert "10000" not in str(info.value)


def test_non_finite_is_rejected_from_toml_text_too() -> None:
    # tomllib produces the float specials from the literal spellings; the
    # check sits in the parser, not the emitter, so the file form is covered.
    text = SPEC_TOML + "\n[prices.override.x]\ninput = inf\noutput = 1\n"
    with pytest.raises(ConfigError, match=r"input must be a finite number"):
        _parse(text)
    text = re.sub(r"cooldown_seconds\s*=\s*60", "cooldown_seconds = nan", SPEC_TOML, count=1)
    assert text != SPEC_TOML, "fixture drift: the spec example no longer sets cooldown_seconds"
    with pytest.raises(ConfigError, match=r"cooldown_seconds must be a finite number"):
        _parse(text)


def test_finite_numbers_still_parse_and_round_trip() -> None:
    raw = _upstream_raw(monthly_budget_usd=1e300, monthly_budget_tokens=2**63, cooldown_seconds=0)
    config = parse_config(raw, "<t>")
    upstream = config.routing.upstream("u")
    assert (upstream.monthly_budget_usd, upstream.monthly_budget_tokens) == (1e300, 2**63)
...
# --- the dataclasses' own helpers (moved in from the pure-logic suite that
# now lives in llm-redact-pro; these need no routing decision) ---------------
...
def test_upstream_config_properties() -> None:
    ...
    # A fresh object per call: a caller merging it into a request body must
    # never share nested containers between requests.
    first = OPENAI_KEY.body_defaults()
    assert first == {"provider": {"only": ["openai"]}}
    assert first is not OPENAI_KEY.body_defaults()


def test_route_rule_is_data_only() -> None:
    # The status -> chain precedence ladder (resolve / chain_for) is routing
    # POLICY and lives in llm-redact-pro: the core dataclass carries the
    # table and nothing that reads it.
    rule = CONFIG.rules[1]
    assert dict(rule.on_status)["throttle_429"] == RETRY_SAME
    assert not hasattr(rule, "resolve") and not hasattr(rule, "chain_for")
