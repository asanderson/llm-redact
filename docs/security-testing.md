...
| B2 | **DNS rebinding**: a hostile page re-resolves its domain to `127.0.0.1`, then reads host-gated metadata (`/recent`, `/sessions`, `/events`, …) or the llm-redact-pro config editor / CSRF token | Local ops surface | Host header must be a known loopback name (403 otherwise), even for GET | `test_b2_hostile_host_rejected` (every host-gated endpoint) |
...
| B9 | A hostile page tries to frame a reserved page (the user guide, the llm-redact-pro dashboard), inject remote script, or leak a Referer | Local ops surface | Strict CSP + `X-Frame-Options: DENY` + nosniff + `Referrer-Policy: no-referrer` on every reserved reply, stamped in one place | `test_b9_security_headers_on_every_reserved_reply` |
...
| B12 | The `/status` or `/metrics` surface leaks a configured value (e.g. an allowlist entry) | Local ops surface | Metadata only: types and counts, never values; the llm-redact-pro config-editor GET is the single documented allowlist exception (pinned in that package) | `test_b12_status_and_metrics_carry_no_values` |
